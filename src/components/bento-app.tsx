...
import { adapterAbi, boxEngineAbi, BOXES, contracts, erc20Abi, ETH_USD_FEED, feedAbi, hasBentoAddresses, hasDeployAddresses, hasZapperAddress, MAG7_COMPONENTS, PLACEHOLDER_ADDRESS, robinhood, USDG_ADDRESS, USDG_DECIMALS, zapperAbi, type BoxInfo } from "@/config/contracts";
...
function BoxCard({ box, deployed }: { box: BoxInfo; deployed: boolean }) {
  const navRead = useReadContract({ address: contracts.boxEngine, abi: boxEngineAbi, functionName: "navUsdPerBox", args: [box.id], query: { enabled: deployed } });
  ...
  const nav = deployed ? formatUsd1e18(navRead.data as bigint | undefined) : "$—.————";
  const tvl = deployed ? formatUsd1e18(tvlUsd) : "—";
  return <Panel className="min-h-[18rem]"><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><div className="relative h-12 w-12 overflow-hidden rounded-xl bg-black"><Image src={box.thumb} alt={`${box.name} thumbnail`} fill sizes="48px" className="object-cover" loading="lazy" /></div><div><h2 className="text-2xl font-semibold text-white">{box.name}</h2><span className="mt-1 inline-flex rounded-full border border-[#f5a623]/20 px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-[#f5a623]">{box.symbol}</span></div></div></div><p className="mt-5 text-sm leading-6 text-zinc-500">{box.description}</p><p className="mt-2 font-mono text-xs text-zinc-600">{box.componentSummary}</p><div className="mt-6 divide-y divide-[#f5a623]/10 border-y border-[#f5a623]/10"><MetricRow label="NAV" value={nav} dim={!deployed} /><MetricRow label="24h change" value="—" dim={!deployed} /><MetricRow label="TVL" value={tvl} dim={!deployed} /></div></Panel>;
}
