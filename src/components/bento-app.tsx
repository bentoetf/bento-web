...
function StatCard({ label, value, caption, dim = false, emptyChart = true }: { label: string; value: string; caption?: string; dim?: boolean; emptyChart?: boolean }) { return <Panel className="grid min-h-28 grid-cols-[minmax(0,1fr)_7.5rem] items-center gap-4 p-5"><div><SectionLabel>{label}</SectionLabel><div className="mt-2"><Value dim={dim}>{value}</Value></div>{caption ? <p className="mt-1 text-xs text-zinc-500">{caption}</p> : null}</div>{emptyChart ? <EmptyChart compact /> : <div />}</Panel>; }
